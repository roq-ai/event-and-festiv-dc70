...
import { getFestivals, deleteFestivalById } from 'apiSdk/festivals';
import { FestivalInterface } from 'interfaces/festival';

type ColumnType = ColumnDef<FestivalInterface, unknown>;
...
export function FestivalListPage(props: FestivalListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getFestivals({
        ...
        order: params.order,
        searchTermKeys: ['title.contains', 'location.contains', 'description.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<FestivalInterface>>(
    () => `/festivals?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: FestivalInterface) => {
    if (hasAccess('festival', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/festivals/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/festivals/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('festival', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/festivals/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('festival', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="festival" />
        </Flex>

        {hasAccess('festival', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/festivals/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
