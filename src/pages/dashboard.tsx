...
import { getFestivals } from 'apiSdk/festivals';
...
function DashboardPage() {
  ...
  const festivalStat = useSWR<PaginatedInterface<FestivalInterface>>(
    () => `/festivals`,
    () => getFestivals({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Festivals',
      result: festivalStat?.data?.totalCount,
      isLoading: festivalStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Products',
